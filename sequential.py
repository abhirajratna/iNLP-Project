"""
Bi-LSTM for Code Authorship Attribution
========================================
Goal: Learn the **lexical fingerprint** of individual programmers.

By training a Bi-LSTM to attribute competitive-programming solutions to their
authors, the model implicitly learns the "rhythm and vocabulary" that
distinguishes each coder's style:
  - Variable naming habits  (camelCase / snake_case / ALL_CAPS)
  - Keyword preferences     (for vs while, cout vs printf, …)
  - Indentation patterns    (tab width, blank-line density, …)
  - Operator & comment style

Dataset : GCJ 2019  (Google Code Jam C++ solutions)
Column  : flines  → raw source code
Label   : username → author to predict

Architecture
------------
  CharEmbedding → Bi-LSTM (N layers) → Attention Pooling
  → [concat hand-crafted lexical features] → Dropout → FC → Softmax
"""
...
class Config:
    ...
    TOP_N_AUTHORS        = 20     # authors with the most samples
    MIN_SAMPLES_PER_AUTHOR = 5   # drop authors below this threshold
    MAX_SEQ_LEN          = 2000  # characters; longer code is truncated

    # ── Lexical hand-crafted features ────────────────────────────────────────
    USE_LEXICAL_FEATURES = True

    # ── Model ─────────────────────────────────────────────────────────────────
    VOCAB_SIZE  = 200   # top-N characters to keep (+ PAD + UNK)
    EMBED_DIM   = 64
    HIDDEN_DIM  = 256
    NUM_LAYERS  = 2
    DROPOUT     = 0.3
...
class LexicalFeatureExtractor:
    """
    Extracts hand-crafted stylistic features from source code.

    These complement the character-level LSTM embeddings by giving the model
    an explicit, interpretable view of each programmer's habits.

    Feature groups
    --------------
    1. Naming conventions   – camelCase, snake_case, ALL_CAPS rates
    2. Keyword frequencies  – normalised counts of 25 C++ keywords/builtins
    3. For-vs-while ratio   – preference for loop style
    4. Indentation style    – tab vs space, average indent depth
    5. Comment style        – line comments (//) vs block comments (/* */)
    6. Line-length stats    – mean, std, max (normalised)
    7. Syntactic density    – brace rate, semicolon rate
    8. Blank-line ratio     – vertical whitespace preference
    9. Short-identifier ratio – preference for terse names (≤2 chars)
    """
...
class CharVocabulary:
    """
    Character-level vocabulary for source code.

    Using characters (rather than words) naturally captures:
      • Raw whitespace patterns  → indentation fingerprint
      • Identifier characters    → naming style
      • Operator spacing         → formatting preference
    """
    ...
    def encode(self, text: str, max_len: int) -> Tuple[List[int], int]:
        """Return (padded id list, true length)."""
...
class CodeStyleDataset(Dataset):
    """
    Each sample is one source-code snippet with an author label.

    Returns a dict with:
      token_ids  : LongTensor  [max_seq_len]
      length     : int         (real sequence length before padding)
      label      : LongTensor  scalar
      lex_feats  : FloatTensor [lex_feature_dim]  (only when extractor is set)
    """
...
class AttentionPooling(nn.Module):
    ...
    Learns which positions (tokens) are most discriminative for identifying
    the author's style, then produces a single weighted context vector.
    """
    ...
    def forward(
        self,
        outputs: torch.Tensor,  # [B, T, H]
        mask:    torch.Tensor,  # [B, T] – True for real positions
    ) -> torch.Tensor:          # [B, H]
        scores  = self.attn(outputs).squeeze(-1)           # [B, T]
        scores  = scores.masked_fill(~mask, float('-inf'))
        weights = torch.softmax(scores, dim=-1)            # [B, T]
        context = (weights.unsqueeze(-1) * outputs).sum(dim=1)  # [B, H]
        return context


# ─────────────────────────────────────────────────────────────────────────────
# Bi-LSTM Style Classifier
# ─────────────────────────────────────────────────────────────────────────────

class BiLSTMStyleClassifier(nn.Module):
    """
    Bidirectional LSTM that learns the lexical fingerprint of code authors.

    The LSTM reads character sequences left-to-right **and** right-to-left,
    allowing it to capture opening patterns (headers, first declarations) and
    closing patterns (return style, last brace) simultaneously.

    Attention pooling focuses on the parts of the code that are most
    author-distinctive rather than simply taking the final hidden state.

    Optionally fuses hand-crafted lexical features before the classifier head
    so that explicit style signals (naming rates, keyword preferences) are
    available alongside the learned representation.
    """
    ...
    def __init__(
        ...
        lstm_out_dim = hidden_dim * 2          # bidirectional doubles width
    ...
    def forward(
        ...
        lex_feats: Optional[torch.Tensor] = None,  # [B, lex_feature_dim]
        ...
        # Boolean mask: True for real (non-padded) positions
        mask    = (
            torch.arange(T, device=token_ids.device).unsqueeze(0)
            < lengths.unsqueeze(1)
        )                                      # [B, T]

        context = self.attn_pool(lstm_out, mask)  # [B, 2*H]

        # Fuse with hand-crafted lexical features
...
def load_data(config: Config):
    """Load CSV(s), filter to C/C++ submissions, deduplicate per (author,task),
    then select top authors and encode labels.
    """
    print(f"Loading from: {config.DATA_PATH}")

    # Support either a single CSV file or a directory containing multiple CSVs.
    if os.path.isdir(config.DATA_PATH):
        import glob
        csv_files = sorted(glob.glob(os.path.join(config.DATA_PATH, "*.csv")))
        if not csv_files:
            raise FileNotFoundError(f"No CSV files found in directory: {config.DATA_PATH}")
        parts = []
        for f in csv_files:
            print(f"  reading: {f}")
    ...
    # Select top-N authors with enough samples
...
def classification_report(preds, labels, class_names):
    """Lightweight per-class precision / recall / F1 report."""
    n     = len(class_names)
    rows  = []
...
def main():
    ...
    print(f"Authors: {config.TOP_N_AUTHORS}  |  Max seq len: {config.MAX_SEQ_LEN} chars\n")
    ...
    train_loader = DataLoader(
        make_ds(train_df), batch_size=config.BATCH_SIZE,
        shuffle=True,  collate_fn=collate_fn, num_workers=2,
    )
    val_loader   = DataLoader(
        make_ds(val_df),   batch_size=config.BATCH_SIZE,
        shuffle=False, collate_fn=collate_fn, num_workers=2,
    )
    test_loader  = DataLoader(
        make_ds(test_df),  batch_size=config.BATCH_SIZE,
        shuffle=False, collate_fn=collate_fn, num_workers=2,
    )
